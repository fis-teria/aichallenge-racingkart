...
class RaceFinishStopLatch:
    """Latch an official per-domain Finish only for the current armed race."""
    ...
    def __init__(self) -> None:
        ...
        self.finish_pending_after_disarm = False
        self.ready_pending_before_arm = False
        self.start_pending_before_arm = False

    def observe_race_armed(self, armed: bool) -> None:
        armed = bool(armed)
        if armed and not self.armed:
            # 新しいrace epochだけが、前回Finishのterminal latchを解除できる。
            self.epoch += 1
            # 通常レースではplanner armを公式Startまで待つ。別subscriptionの
            # callback順によりReady->Startがarmより先に届いても、同じ公式状態列を
            # 現在epochへ引き継ぎ、Finish停止の資格を失わない。
            self.ready_seen = self.ready_pending_before_arm or (
                self.last_vehicle_state == "Ready"
            )
            self.race_started = (
                self.ready_seen and self.start_pending_before_arm
            )
            self.latched = False
            self.finish_pending_after_disarm = False
            self.ready_pending_before_arm = False
            ...
            if self.race_started:
                ...
                self.phase = "ready" if self.ready_seen else "armed"
        elif not armed and not self.latched:
            self.finish_pending_after_disarm = self.race_started
            self.phase = "disarmed"
        # Finish直後のdisarmではlatchedを保持し、停止commandを継続する。
        self.armed = armed

    def observe_vehicle_state(self, state: str) -> None:
        normalized = str(state).strip()
        # armed前のReadyも記録する。Finishをlatchedする条件は現在epochで
        # Ready->Startまで観測済みであることに限定する。
        self.last_vehicle_state = normalized
        if not self.armed:
            if normalized == "Ready":
                self.ready_pending_before_arm = True
                self.start_pending_before_arm = False
            elif normalized == "Start" and self.ready_pending_before_arm:
                self.start_pending_before_arm = True
            elif normalized not in ("Finish",):
                # Spawned/Grounded等が来たら、過去epochの順序を持ち越さない。
                self.ready_pending_before_arm = False
                self.start_pending_before_arm = False
        # 実bagの記録順はFinish->race_armed=falseでも、別subscriptionの
        # callback実行順は逆転し得る。現在epochでStartまで確認済みなら、
        # 直後disarmとの順序に依存せず公式Finishをterminal停止へlatchedする。
        if normalized == "Finish" and self.race_started and (
            self.armed or self.finish_pending_after_disarm
        ):
            self.latched = True
            self.finish_pending_after_disarm = False
            self.ready_pending_before_arm = False
            ...
            self.phase = "finished"
            return
        if normalized != "Finish" and not self.armed:
            self.finish_pending_after_disarm = False
        ...
        if normalized == "Ready":
            ...
            self.phase = "ready"
            return
        if normalized == "Start" and self.ready_seen:
...
class HybridMuxConfig:
    ...
    state_lattice_instant_control_enabled: bool = False
...
class RosClockProgressWatchdog:
    ...
    def reset(self) -> None:
        """Forget the previous epoch so its frozen stamp cannot taint a new race."""
        self._maximum_ros_time_ns = None
        self._last_progress_steady_sec = None
...
class SteeringLimiterConfig:
    ...
    max_steering_rate_radps: float = 128.0
...
class HybridMuxCore:
    def __init__(self, config: HybridMuxConfig) -> None:
        ...
        self.state_lattice_episode_latched = False
    ...
    def update(
        ...
        *,
        ...
        state_lattice: StateLatticeMuxState | None = None,
    ) -> MuxDecision:
        ...
        state_lattice_decision = self._update_state_lattice(
            state_lattice=state_lattice
        )
        if state_lattice_decision is not None:
            return state_lattice_decision
    ...
    def _update_state_lattice(
        self, *, state_lattice: StateLatticeMuxState | None
    ) -> MuxDecision | None:
        """Select the dedicated instantaneous source only during its episode.

        A fresh active=false sample is the only normal handoff back to the
        configured MPC/PP primary.  Once an active episode has been observed,
        missing or invalid samples stop instead of silently switching control
        laws underneath the vehicle.
        """
        if (
            state_lattice is None
            or not self.config.state_lattice_instant_control_enabled
            or not state_lattice.enabled
        ):
            self.state_lattice_episode_latched = False
            return None

        if state_lattice.command_fresh and state_lattice.active:
            self.state_lattice_episode_latched = True

        if not self.state_lattice_episode_latched:
            return None

        if not state_lattice.command_fresh:
            return MuxDecision(
                "stop", True, "state_lattice_command_timeout", self.solved_cycles
            )

        if not state_lattice.active and state_lattice.command_valid:
            self.state_lattice_episode_latched = False
            self.last_reason = "state_lattice_handoff"
            return None

        if not state_lattice.active:
            return MuxDecision(
                "stop",
                True,
                "state_lattice_invalid_handoff",
                self.solved_cycles,
            )

        gates_ok = (
            state_lattice.authority_valid
            and state_lattice.command_valid
            and state_lattice.command_forward_only
        )
        if gates_ok:
            self.last_reason = "state_lattice_instant_active"
            return MuxDecision(
                "state_lattice", False, self.last_reason, self.solved_cycles
            )
        return MuxDecision(
            "stop", True, "state_lattice_gate_not_satisfied", self.solved_cycles
        )
