...
DESCRIPTION_DEFAULTS: dict[str, str] = {
    ...
    "overtake_planner_node.ros__parameters.parallel_follow_enabled": "same_corridor外の前方近接parallel車を、現在d保持FOLLOW候補に追加するテスト機能です。既存挙動維持ならfalseにします。",
    "overtake_planner_node.ros__parameters.parallel_follow_s_m": "parallel FOLLOW候補として見る前方距離[m]です。短いほど近いparallel車だけを車間形成対象にします。",
    "overtake_planner_node.ros__parameters.parallel_follow_lateral_width_m": "parallel FOLLOW候補に含める最大横差[m]です。same_corridor_width_mより広い分だけ追加FOLLOW対象になります。",
    ...
    "wall_recovery_planner_node.ros__parameters.wall_recovery_enabled": "壁接触後スタック復帰plannerを有効にするフラグです。安全入力とRecoveryPermitが揃うまで通常はfalseのままにします。",
    "wall_recovery_planner_node.ros__parameters.allow_unverified_recovery": "RecoveryPermitなしの復帰開始を許す実験用フラグです。安全証明を迂回するため通常はfalse固定です。",
    "wall_recovery_planner_node.ros__parameters.require_recovery_permit": "overtake planner等が出すRecoveryPermitを復帰開始・継続に必須とする設定です。trueでfail-closedになります。",
    "wall_recovery_planner_node.ros__parameters.control_rate_hz": "wall recovery plannerの状態判定とstatus publish周期[Hz]です。高いほど反応は細かくなりますが負荷が増えます。",
    "wall_recovery_planner_node.ros__parameters.collision_edge_min_delta": "pitstop conditionの増分を衝突候補edgeとみなす最小値です。小さいほど接触検出に敏感になります。",
    "wall_recovery_planner_node.ros__parameters.collision_event_ttl_sec": "衝突候補edgeをスタック判定に使える有効時間[s]です。短いほど古い接触情報で復帰しにくくなります。",
    "wall_recovery_planner_node.ros__parameters.stuck_confirm_sec": "衝突候補、前進指令、低速、無進捗が連続成立してからスタックと確定する時間[s]です。",
    "wall_recovery_planner_node.ros__parameters.stuck_speed_threshold_mps": "スタック判定で自車がほぼ停止しているとみなす実速度上限[m/s]です。",
    "wall_recovery_planner_node.ros__parameters.stuck_progress_threshold_m": "スタック確認窓で許す位置進捗の上限[m]です。これ以上動いた場合は復帰対象外に戻します。",
    "wall_recovery_planner_node.ros__parameters.stuck_min_command_speed_mps": "通常制御が前進を要求しているとみなす最小速度指令[m/s]です。",
    "wall_recovery_planner_node.ros__parameters.stop_hold_sec": "スタック確定後、Recovery trajectoryへ移る前に停止保持する時間[s]です。",
    "wall_recovery_planner_node.ros__parameters.permit_wait_timeout_sec": "candidate trajectory発行後にRecoveryPermitを待つ最大時間[s]です。超えるとlockoutします。",
    "wall_recovery_planner_node.ros__parameters.recovery_v_max_mps": "壁復帰trajectoryへ設定する最大速度[m/s]です。mux側のrecovery_speed_mps以下に揃えます。",
    "wall_recovery_planner_node.ros__parameters.recovery_max_duration_sec": "1回のRecovery ACTIVEを許す最大時間[s]です。超えると通常制御へ戻さずlockoutします。",
    "wall_recovery_planner_node.ros__parameters.recovery_no_progress_timeout_sec": "Recovery中に進捗がない状態を許す最大時間[s]です。超えると押し続け防止でlockoutします。",
    "wall_recovery_planner_node.ros__parameters.recovery_max_attempts": "同一起動中に許すRecovery試行回数です。初期実装では繰り返し押し続けを避けるため1が安全側です。",
    "wall_recovery_planner_node.ros__parameters.max_odom_age_sec": "復帰判定でodometryを新鮮とみなす最大時間[s]です。古い自己位置ではfail-closedになります。",
    "wall_recovery_planner_node.ros__parameters.max_trajectory_age_sec": "基準Trajectoryを新鮮とみなす最大時間[s]です。古い経路では復帰目標を作りません。",
    "wall_recovery_planner_node.ros__parameters.max_control_cmd_age_sec": "最終制御指令を新鮮とみなす最大時間[s]です。前進指令確認に使います。",
    "wall_recovery_planner_node.ros__parameters.max_permit_age_sec": "RecoveryPermitを新鮮とみなす最大時間[s]です。古い安全許可では復帰を開始・継続しません。",
    "wall_recovery_planner_node.ros__parameters.min_rejoin_arc_distance_m": "復帰先waypointとして選ぶ最小前方arc距離[m]です。近すぎる点への急旋回を避けます。",
    "wall_recovery_planner_node.ros__parameters.max_rejoin_arc_distance_m": "復帰先waypointとして選ぶ最大前方arc距離[m]です。遠すぎる点へ直線的に向かわないための上限です。",
    "wall_recovery_planner_node.ros__parameters.max_rejoin_heading_error_rad": "復帰先waypointと自車yawの許容heading差[rad]です。大きいほど向き違いの点も候補になります。",
    "wall_recovery_planner_node.ros__parameters.max_trajectory_segment_length_m": "基準Trajectoryの点間距離が不連続とみなされる上限[m]です。周回境界や壊れた経路をまたがないための保護です。",
    "wall_recovery_planner_node.ros__parameters.recovery_trajectory_points": "生成するRecovery trajectoryの点数です。多いほどPure Pursuitが追いやすくなりますが冗長になります。",
    "wall_recovery_planner_node.ros__parameters.handoff_distance_threshold_m": "Recovery trajectory終端へ到達したとみなす距離[m]です。小さいほど復帰完了判定が厳しくなります。",
    "wall_recovery_planner_node.ros__parameters.complete_publish_window_sec": "COMPLETE statusをpublishし続ける時間[s]です。muxがhandoffを確認する余裕になります。",
    "wall_recovery_planner_node.ros__parameters.debug_publish_period_sec": "/wall_recovery/debugをpublishする周期[s]です。0以下でdebug出力を抑制します。",
    ...
    "hybrid_control_mux_node.ros__parameters.recovery_enabled": "wall recovery plannerからのRecovery sourceをmuxで有効にするフラグです。falseなら通常MPC/PP仲裁だけを行います。",
    "hybrid_control_mux_node.ros__parameters.recovery_cmd_timeout_sec": "Recovery専用Pure PursuitのRecoveryControlCommandを新鮮とみなす最大時間[s]です。超えるとRecovery中はstopになります。",
    "hybrid_control_mux_node.ros__parameters.recovery_status_timeout_sec": "RecoveryStatusを新鮮とみなす最大時間[s]です。Recovery episode中に超えるとMPC/PPへ戻さずstopします。",
    "hybrid_control_mux_node.ros__parameters.recovery_permit_timeout_sec": "RecoveryPermitを新鮮とみなす最大時間[s]です。古い安全許可ではRecovery commandを採用しません。",
    "hybrid_control_mux_node.ros__parameters.external_safety_timeout_sec": "外部安全statusを新鮮とみなす最大時間[s]です。必須設定時に欠落・staleならRecoveryを止めます。",
    "hybrid_control_mux_node.ros__parameters.require_recovery_external_safety_status": "Recovery中にSafetyStopStatusの明示的な非停止状態を必須にする設定です。trueがfail-closed側です。",
    "hybrid_control_mux_node.ros__parameters.recovery_speed_mps": "mux下流でRecovery commandへ強制する速度上限[m/s]です。wall_recovery_planner側のrecovery_v_max_mps以下に揃えます。",
    "hybrid_control_mux_node.ros__parameters.recovery_accel_max_mps2": "mux下流でRecovery commandへ許す最大加速度[m/s^2]です。",
    "hybrid_control_mux_node.ros__parameters.recovery_decel_min_mps2": "mux下流でRecovery commandへ許す最小加速度[m/s^2]です。負の値で減速側を表します。",
    "hybrid_control_mux_node.ros__parameters.recovery_max_duration_sec": "muxがRecovery episodeを許す最大時間[s]です。超えるとRecovery commandを採用せずstopします。",
...
DELAY_AWARE_XML_DEFAULTS: dict[str, str] = {
    ...
    "input_recovery_cmd": "hybrid control muxが受け取るRecovery専用Pure Pursuitの型付き制御指令topicです。",
    "input_recovery_status": "Recovery専用Pure Pursuitやmuxが参照するRecoveryStatus topicです。attempt/generation一致確認に使います。",
    "input_external_safety_status": "muxがRecovery中に監視する外部安全status topicです。未接続を許すかはrequire_recovery_external_safety_statusで決めます。",
    "output_recovery_control_cmd": "Recovery専用Pure Pursuitがpublishするgeneration付きRecoveryControlCommand topicです。",
    "param_file": "ノードへ読み込ませるYAMLパラメータファイルです。launchから読み込む設定一式を切り替えます。",
    ...
    "input_pitstop_condition": "wall recovery plannerが接触候補edgeとして監視するpitstop condition topicです。",
    "input_reference_trajectory": "wall recovery plannerが復帰先waypointを選ぶ基準Trajectory topicです。",
    "input_control_cmd": "wall recovery plannerが前進指令の有無を確認する最終制御指令topicです。",
    "input_recovery_permit": "wall recovery planner/muxが参照する型付きRecoveryPermit topicです。候補trajectoryの安全許可を受け取ります。",
    "overtake_override_timeout_sec": "Pure Pursuit側でovertake reference overrideを新鮮とみなす最大時間[s]です。古い追い越し指令を使い続けないための上限です。",
    "recovery_mode": "Pure PursuitをRecovery専用インスタンスとして動かすフラグです。trueでは通常AckermannではなくRecoveryControlCommandを出します。",
    "recovery_status_timeout_sec": "Recovery専用Pure PursuitがRecoveryStatusを新鮮とみなす最大時間[s]です。",
    "wall_recovery_enabled": "壁接触後スタック復帰plannerとmuxのRecovery入力を有効にするlaunch引数です。検証前はfalse推奨です。",
    "output_candidate_trajectory": "wall recovery plannerが安全評価へ出す候補RecoveryTrajectory topicです。",
    "output_trajectory": "Recovery専用Pure Pursuitが追従する許可済みTrajectory topicです。",
    "output_status": "wall recovery plannerがmuxへ出すRecoveryStatus topicです。状態、attempt、generation、stop要求を含みます。",
...
CATALOG: dict[str, list[dict[str, str]]] = {
    ...
    "hybrid_delay_aware_mpc": [
        ...
        },
        {
            ...
            "path": str(WALL_RECOVERY_ROOT / "config/wall_recovery_planner.param.yaml"),
            "kind": "yaml",
        },
        {
            "label": "Wall recovery planner launch",
            "path": str(WALL_RECOVERY_ROOT / "launch/wall_recovery_planner.launch.xml"),
            "kind": "xml",
        },
        {
            "label": "Delay-aware MPC launch params",
