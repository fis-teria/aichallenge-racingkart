...
- `control_method` が `state_lattice_pure_pursuit` のときは、MPCを起動せず、State Latticeの安全評価済みreference overrideをPure Pursuitが追従し、Hybrid Muxだけが最終commandをpublishします。Filesには専用profile/base launch、Planner、Pure Pursuit、Mux、Wall Recovery、追越許可CSV、参照ラインCSV、壁マップYAMLを表示します。State Latticeの追越許可CSVも既存plannerと同じ表編集・保存時検証を使います。各parameterのdescriptionには単位・調整方向・安全上の役割を表示し、`control_method`、`control_route`、`trajectory_backend`などの選択式には対応するprofile/backend名も表示します。経路選択はDocker環境変数ではなく、`Overtake planner route params` の `control_route: state_lattice_pure_pursuit` が所有します。
