...
// Minimal V4 live-path PoC binding.  This intentionally reuses the accepted
// typed V2 proposal identity instead of extending the Float32 V4 wire format.
// It is not the final M4 identity contract: it only proves that the V4
// generation and the PP's current base trajectory came from the same bounded
// planner/PP exchange.
inline bool v4PocIdentityMatches(
    const multi_purpose_mpc_ros_msgs::msg::StateLatticeV2Identity &identity,
    std::uint32_t override_generation, const std::string &base_frame_id,
    const builtin_interfaces::msg::Time &base_stamp,
    std::uint32_t base_source_generation) {
  return override_generation > 0U &&
         identity.plan_generation == override_generation &&
         !base_frame_id.empty() && identity.frame_id == base_frame_id &&
         identity.source_stamp == base_stamp && base_source_generation > 0U &&
         identity.source_generation == base_source_generation;
}
