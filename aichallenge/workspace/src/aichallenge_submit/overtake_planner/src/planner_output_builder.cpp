...
// 処理概要:
// 不安全候補から速度だけfallbackする時に、元候補の速度意図をできるだけ残す。
...
bool hasPositiveSpeedCap(const std::vector<double> &speed_caps) {
  return std::any_of(
      speed_caps.begin(), speed_caps.end(), [](double speed_cap_mps) {
        return std::isfinite(speed_cap_mps) && speed_cap_mps > 0.0;
      });
}
...
// 入力: 要素数countと値value。
// 出力: 少なくとも1要素を持つ一様配列。
// 処理概要: 速度だけguardする時でもMPC
// overrideプロトコルに必要な配列長を確保する。
...
// 処理概要: ABORT_RECOVERYまたはRECOVERY候補では、壁/health/lateral
// errorに応じて速度を落とす。
...
bool isPassControlMode(BehaviorMode mode) {
...
// 出力: opponent_collision
// fallbackを最低速側へ倒すほど近い相手車リスクならtrue。 処理概要:
// parallel_side_candidateは広く拾う診断値なので、横楕円/横並び幅の外に
...
bool shouldUseStrictOpponentCollisionFallback(const PlannerConfig &config,
  ...
  return blocked.side_by_side || blocked.corner_side_by_side ||
         ...
         blocked.future_yield_required || blocked.future_parallel_interaction ||
...
std::string authoritativeTargetVehicleId(const BlockedInfo &blocked) {
  if (blocked.maneuver_target_latched && !blocked.maneuver_target_id.empty()) {
    return blocked.maneuver_target_id;
  }
  if (!blocked.start_grid_target_id.empty()) {
    return blocked.start_grid_target_id;
  }
  for (const auto *candidate_id :
       {&blocked.nearest_id, &blocked.early_stationary_parallel_pass_id,
        &blocked.stationary_front_id, &blocked.parallel_follow_id,
        &blocked.side_id}) {
...
bool authoritativePlanTrajectoryPayloadRequired(
  ...
  (void)lateral_maneuver_required;
  return trajectory_authorized;
}

// 入力: planner設定。
// 出力: PlannerOutput生成器のインスタンス。
// 処理概要: 候補選択結果に速度ガードやsafe
// stop情報を重ねるための設定を保持する。
PlannerOutputBuilder::PlannerOutputBuilder(const PlannerConfig &config)
    : config_(config) {}

// 入力: mode、選択候補、安全停止文脈、section profile、MPC
// healthなどの集約入力。 出力: ROS nodeがpublishしやすい平坦なPlannerOutput。
// 処理概要: 候補評価結果にsafe
// stop理由と速度ガードを重ね、最終override配列とdebug指標を作る。
PlannerOutput
PlannerOutputBuilder::build(const PlannerOutputBuildInput &input) const {
  ...
  const bool evaluated_recovery_lateral_safe =
      input.safe_stop_candidate_infeasible &&
      selected.type == CandidateType::RECOVERY && selected.feasible &&
      selected.safety_evaluated && selected.pass_target_corridor_valid &&
      selected.controller_tracking_profile_valid &&
      selected.longitudinal_profile_valid && selected.d.size() > 1U &&
      selected.d.size() == selected.v_ref.size() &&
      selected.d.size() == selected.longitudinal_offsets_m.size() &&
      std::all_of(selected.d.begin(), selected.d.end(),
                  [](double value) { return std::isfinite(value); }) &&
      std::all_of(
          selected.v_ref.begin(), selected.v_ref.end(),
          [](double value) { return std::isfinite(value) && value >= 0.0; }) &&
      std::all_of(selected.longitudinal_offsets_m.begin(),
                  selected.longitudinal_offsets_m.end(), [](double value) {
                    return std::isfinite(value) && value >= 0.0;
                  });
  bool selected_override_active = false;
  // 処理ブロック: 選択候補自体が横/速度overrideを持つかを決める。
  // 設計意図:
  // FASTESTは通常走行なのでoverrideしないが、SAFE_STOPだけは停止候補として明示的に出す。
  if (selected.type == CandidateType::SAFE_STOP) {
    selected_override_active = output.selected_lateral_profile_safety_verified;
  } else {
    selected_override_active = selected.type != CandidateType::FASTEST &&
                               selected.feasible &&
                               (!input.safe_stop_candidate_infeasible ||
                                evaluated_recovery_lateral_safe);
  }
  ...
  // 設計意図: 壁リスク、MPC
  // health、fallbackが同時に出ても、より安全側の速度を1つに集約する。
        ...
        if (reject_reason == "opponent_collision") {
          if (config_.side_by_side_leader_priority_enabled &&
            ...
            cap = finitePositiveOr(
                config_.side_by_side_leader_priority_v_max_mps, cap);
  ...
  if (allow_speed_guard && config_.speed_only_fallback_enabled &&
    ...
    requestSpeedCap(requested_fallback_cap,
                    leader_priority_relaxed
  ...
  const bool clean_free_run_context =
      input.mode == BehaviorMode::FREE_RUN &&
      selected.type == CandidateType::FASTEST && selected.feasible &&
      !safe_stop_context.requested && !input.safe_stop_candidate_infeasible &&
      !blocked.blocked && !blocked.side_by_side &&
      !blocked.corner_side_by_side && !blocked.parallel_side_candidate &&
      !blocked.parallel_follow_candidate && !blocked.start_grid_target_active &&
      ...
      blocked.ego_wall_clearance_m >= config_.wall_soft_margin_m;
  const bool clean_free_run_with_fresh_pp =
      config_.mpc_health_clean_free_run_soft_guard_bypass_enabled &&
      input.pure_pursuit_primary_and_fresh && clean_free_run_context;
  const bool non_pass_motion_context =
      selected.feasible && !isPassCandidate(selected.type) &&
      !isPassControlMode(input.mode) &&
      !blocked.maneuver_transaction_incomplete &&
      !safe_stop_context.requested && !input.safe_stop_candidate_infeasible;
  const bool verified_inactive_mpc_non_pass =
      config_.mpc_health_inactive_mpc_free_run_bypass_enabled &&
      input.verified_non_mpc_pure_pursuit && non_pass_motion_context;
  const bool mpc_health_soft_guard =
      (mpc_health_infeasible_soft_guard || mpc_health_solve_time_guard) &&
      !clean_free_run_with_fresh_pp && !verified_inactive_mpc_non_pass;
  const bool mpc_health_hard_or_stale_guard =
      (mpc_health_infeasible_guard || mpc_health_stale_guard) &&
      !verified_inactive_mpc_non_pass;
  const bool mpc_health_speed_guard_condition =
      mpc_health.valid &&
      (mpc_health_hard_or_stale_guard || mpc_health_soft_guard);
  const bool mpc_health_speed_guard_applies =
      config_.mpc_health_speed_guard_enabled &&
      mpc_health_speed_guard_condition;
  if (allow_speed_guard && mpc_health_speed_guard_applies) {
    const std::string reason =
        mpc_health_infeasible_guard && !verified_inactive_mpc_non_pass
            ? "mpc_health_infeasible_guard"
        : mpc_health_infeasible_soft_guard && !clean_free_run_with_fresh_pp &&
                !verified_inactive_mpc_non_pass
            ? "mpc_health_infeasible_soft_guard"
        : mpc_health_solve_time_guard && !clean_free_run_with_fresh_pp &&
                !verified_inactive_mpc_non_pass
  ...
  const bool recovery_target_valid = selected.type == CandidateType::RECOVERY &&
                                     selected.feasible && !selected.d.empty() &&
                                     std::isfinite(selected_target_d);
  const double recovery_tracking_target_d =
      recovery_target_valid ? selected_target_d : 0.0;
  const double recovery_tracking_error =
      std::isfinite(input.ego.frenet.d)
          ? std::abs(input.ego.frenet.d - recovery_tracking_target_d)
          : std::numeric_limits<double>::infinity();
  const bool large_recovery_tracking_error =
      config_.large_lateral_error_threshold_m >= 0.0 &&
      recovery_tracking_error > config_.large_lateral_error_threshold_m;
  if (allow_speed_guard && config_.recovery_speed_guard_enabled &&
      ...
      (wall_risk_speed_guard_applies || mpc_health_speed_guard_applies ||
       large_recovery_tracking_error)) {
    const std::string reason =
        mpc_health_speed_guard_applies  ? "recovery_mpc_health_speed_guard"
        : wall_risk_speed_guard_applies ? "recovery_wall_risk_speed_guard"
  ...
  // 処理ブロック:
  // 選択候補をPlannerOutputへコピーし、必要なら速度ガードを重ねる。 設計意図:
  // 横overrideがある場合は横列を維持する。横軌道が安全評価を通らない
  ...
  output.longitudinal_offsets_m = selected.longitudinal_offsets_m;
  output.required_controller_spatial_horizon_m =
      selected.required_controller_spatial_horizon_m;
  ...
  if (selected_override_active) {
    if (selected.type == CandidateType::PASS_LEFT ||
      ...
      // PREPARE中でも実際にPASS候補をauthoritative lateral overrideとして
      // publishする設定では、mode名ではなくpayload型に従ってmaneuver intentを
      // 明示する。FOLLOWをpublishするovertake_only設定はこの条件へ入らない。
  ...
  output.recovery_tracking_error_m = recovery_tracking_error;
  output.recovery_tracking_target_d_m = recovery_tracking_target_d;
